...
export default function TanzaniaSafariPage() {
  ...
  const [selectedRoutes, setSelectedRoutes] = useState<string[]>(['camping-4day', 'camping-8day', 'safari-kili-6day', 'luxury-safari'])
  const monthDropdownRef = useRef<HTMLDivElement>(null)
  const t = useTranslations('TanzaniaSafariPage')
  ...
  // Actual safari packages available
  ...
  const routeOptions = safariPackages.map(pkg => ({
    id: pkg.id,
    label: pkg.name
  }))
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="text-white">
              ...
              <div className="flex items-center mb-4">
                ...
                <span className="text-lg">Trips/Tanzania Safari</span>
              </div>
      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
            ...
            {/* Materuni & Chemka Cultural Experience Card */}
            ...
            {/* Safari & Kilimanjaro Adventure Card */}
      ...
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4 max-w-6xl">
            ...
            <div className="relative flex-1">
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  <span className="font-semibold">{selectedRoutes.length} Selected</span>
                </span>
                <svg className={`w-4 h-4 transition-transform ${isRouteDropdownOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              ...
              {isRouteDropdownOpen && (
                <div className="absolute z-20 mt-2 w-full bg-white rounded-lg shadow-xl border border-gray-200 p-3">
                  {routeOptions.map((opt) => {
                    const isSelected = selectedRoutes.includes(opt.id);
                    return (
                      <label key={opt.id} className="flex items-center gap-2 p-2 rounded hover:bg-gray-50 cursor-pointer">
                        <input 
                          ...
                          onChange={() => {
                            if (isSelected) {
                              setSelectedRoutes(selectedRoutes.filter(s => s !== opt.id));
                            } else {
                              setSelectedRoutes([...selectedRoutes, opt.id]);
                            }
                          }} 
          ...
          <div className="space-y-3 mb-6">
            {safariPackages
              .filter(pkg => selectedRoutes.length === 0 || selectedRoutes.includes(pkg.id))
              .map((pkg) => (
