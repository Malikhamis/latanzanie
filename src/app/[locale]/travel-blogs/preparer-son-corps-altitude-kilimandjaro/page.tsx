...
export default function PrepareBodyAltitudePage({ params }: { params: { locale?: string } }) {
  const locale = useLocale() || params?.locale || 'fr'
  const t = useTranslations('BlogPosts.preparer-son-corps-altitude-kilimandjaro')
  const idsLocal = ids
  const sections = idsLocal.map((id) => ({
    id,
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="py-12 border-b border-gray-200">
        <div className="container mx-auto px-4 max-w-4xl">
          <AuthorMeta author={locale === 'fr' ? 'Guide Local Kilimandjaro' : 'Kilimanjaro Local Guide'} date={locale === 'fr' ? 'Décembre 2025' : 'December 2025'} />
        </div>
      </section>
          ...
          <div className="max-w-7xl mx-auto md:flex md:gap-8">
            ...
            <div className="flex-1 space-y-6">
              <div className="mb-8">
                ...
                <p className="text-base md:text-lg text-black max-w-3xl">{locale === 'fr' ? 'Conseils pour se préparer à l’altitude et améliorer vos chances au sommet.' : 'Advice to prepare for altitude and improve your summit chances.'}</p>
              </div>
