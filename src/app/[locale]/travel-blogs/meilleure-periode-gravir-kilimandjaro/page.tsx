...
const contentFr = `Quelle est la meilleure période pour faire l’ascension du Kilimandjaro ?
...
En résumé, janvier à début mars est une période parfaite pour toutes ces routes. Que vous choisissiez Machame, Lemosho ou Marangu, vous bénéficierez de sentiers praticables, d'une météo stable et de panoramas spectaculaires, ce qui rend l'expérience sur le Kilimandjaro encore plus mémorable.


---

🌤️ 2. Fin juin à octobre — La grande saison sèche du Kilimandjaro
...
🌧️ La saison des pluies sur le Kilimandjaro
...
const contentEn = `What is the best time to climb Mount Kilimanjaro?

Complete guide written by a local guide in Tanzania
...
January to early March: the "warm" dry season

The period from January to early March is considered one of the best times to climb Kilimanjaro. During these months, the weather is generally stable and mornings are often clear, offering exceptional visibility of the summit and glaciers. Temperatures are milder in the lower and intermediate zones, making the walk more comfortable and less tiring, even though nights remain cold at altitude.
...
Why choose Machame, Lemosho or Marangu in January to early March?

Machame: the spectacular "Whiskey" route
...
In summary, January to early March is a perfect period for all these routes. Whether you choose Machame, Lemosho or Marangu, you will benefit from passable trails, stable weather, and spectacular panoramas, making the Kilimanjaro experience even more memorable.


---

🌤️ 2. Late June to October — Kilimanjaro's great dry season

The most stable and recommended period for ascent
...
function processRouteLinks(text: string, locale: string) {
  ...
  const processedParts = parts.map((part, index) => {
    ...
    if (lowerPart === 'machame') {
      ...
      while ((seasonalMatch = seasonalRegex.exec(part)) !== null) {
        // Add text before the match
        if (seasonalMatch.index > lastSeasonalIndex) {
          seasonalParts.push(part.substring(lastSeasonalIndex, seasonalMatch.index));
        }
      ...
      if (lastSeasonalIndex < part.length) {
        seasonalParts.push(part.substring(lastSeasonalIndex));
      }
  ...
  while (i < lines.length) {
    if (lines[i].startsWith('>')) {
      ...
      if (text.startsWith('#')) {
        ...
        // Process route links in emphasized text
        const processedText = processRouteLinks(emphasizedText, locale);
        nodes.push(<p key={keyIndex++} className="mb-4 font-semibold">{processedText}</p>);
        ...
        // Process route links in regular paragraphs
        const processedText = processRouteLinks(text, locale);
        nodes.push(<p key={keyIndex++} className="mb-4">{processedText}</p>)
      }
    } else {
      i++
    }
...
export default function MeilleurePeriodeGravirKilimandjaroPage() {
  const locale = useLocale()
  const t = useTranslations('BlogPosts.meilleure-periode-gravir-kilimandjaro')
  const title = t('title')
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto md:flex md:gap-8">
            ...
            <div className="flex-1 space-y-6">
              <div className="mb-8">
                <h1 className="text-3xl md:text-4xl font-bold mb-4 leading-tight">{locale === 'fr' ? 'Quelle est la meilleure période pour faire l’ascension du Kilimandjaro ?' : 'What is the best time to climb Mount Kilimanjaro?'}</h1>
                <p className="text-base md:text-lg text-gray-600 max-w-3xl">{locale === 'fr' ? 'Guide complet par un guide local en Tanzanie.' : 'Complete guide by a local guide in Tanzania.'}</p>
              </div>
